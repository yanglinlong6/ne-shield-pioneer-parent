 ...
 */
public class DateUtil extends com.glsx.plat.common.utils.DateUtils {
     ...
     * @return
     */
    public static String formatDateChineseYmdh(Date date) {
        return formatDate(date, "yyyy年MM月dd日HH时");
    }
     ...
     * @return
     */
    public static String formatDateChineseYmdhms(Date date) {
    ...
    public static Date getBeforeOrAfterTime(Date date, TimeUnitType timeUnitType, int num) {
    ...
    public static String getBeforeOrAfterTimeString(Date date, TimeUnitType timeUnitType, int num) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
        if (TimeUnitType.SECOND.value() == timeUnitType.value()) {
            cal.add(Calendar.SECOND, num);
    ...
    public static List<List<String>> getStaticYMD(Date startDate, Date endDate) {
        List<List<String>> dateStr = new ArrayList<>();
        // 获取两个日期的天数差
        long daydiff = getDiffDays(startDate, endDate);
        dateStr.add(Lists.newArrayList(DateUtil.formatDateYmd(startDate)));
        for (int i = 1; i < daydiff; i++) {
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(startDate);
            calendar.add(Calendar.DAY_OF_MONTH, i);
            dateStr.add(Lists.newArrayList(DateUtil.formatDateYmd(calendar.getTime())));
        }
        //如果开始时间和结束时间是同一天。只取一天
        if (Objects.equals(DateUtil.formatDateYmd(startDate), DateUtil.formatDateYmd(endDate))) {
            return dateStr;
        }
        dateStr.add(Lists.newArrayList(DateUtil.formatDateYmd(endDate)));
        return dateStr;
    }
...
//    	String preMonday = df.format(monday);
     ...
     * @return
     */
    public static String formatTimeInSec1(long durationTime) {
        if (durationTime > 0) {
            int s = (int) durationTime;
            int day = s / (3600 * 24);
            s = s % (3600 * 24);
     ...
     * @return String 返回值为：xx天xx小时xx分xx秒
     */
    public static String getDistanceTimeIgnoreZero(String str1, String str2) {
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date one;
        Date two;
        try {
            ...
            long min = ((diff / (60 * 1000)) - day * 24 * 60 - hour * 60);
            long sec = (diff / 1000 - day * 24 * 60 * 60 - hour * 60 * 60 - min * 60);
            return getIgnoreZero(day, "天") +
                    getIgnoreZero(hour, "小时") +
                    getIgnoreZero(min, "分") +
                    getIgnoreZero(sec, "秒");
        } catch (ParseException e) {
            e.printStackTrace();
     ...
     * 去掉0
     */
    private static String getIgnoreZero(long number, String unit) {
        if (number == 0) {
            return "";
        }
        return number + unit;
...
//    }
    ...
    public static int getMaxDayOfNow(Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
    ...
    public static int getMaxDayOfCurrentMonth() {
        Calendar cal = Calendar.getInstance();
        int maxNum = cal.getMaximum(Calendar.DAY_OF_MONTH);
        return maxNum;
    }


    public static String getYearAndMonthYesterday() {
        Calendar cal = Calendar.getInstance();
        cal.add(Calendar.DATE, -1);
        String yesterday = new SimpleDateFormat("yyyy-MM").format(cal.getTime());
        return yesterday;
    }
     ...
     * HH 时 mm 分 ss 秒 时长
     *
     * @param timestamp 时长时间戳
     * @return HH 时 mm 分 ss 秒 时长
     */
    public static String normalFormatDuration(Long timestamp) {
        if (timestamp == 0L) {
            return "0 秒";
        }
        if (timestamp < 0L) {    //小于0不合法
            return "";
        }
        if (timestamp < org.apache.commons.lang.time.DateUtils.MILLIS_PER_HOUR) {
            return DurationFormatUtils.formatDuration(timestamp, "mm 分 ss 秒");
        }
        return DurationFormatUtils.formatDuration(timestamp, "HH 时 mm 分 ss 秒");
    }
    ...
    public static Long getTimestampMillisecond(String datetimeStr) {
     ...
     * @return yyyy-MM-dd HH:mm:ss
     */
    public static String getAfterDateTimeStr(String dateTime1, String dateTime2) {
        if (StringUtils.isEmpty(dateTime1) && StringUtils.isEmpty(dateTime1)) {
            return "";
        }
        if (StringUtils.isEmpty(dateTime1)) {
            return dateTime2;
        }
        if (StringUtils.isEmpty(dateTime2)) {
            return dateTime1;
        }
        LocalDateTime localDateTime1 = LocalDateTime.parse(dateTime1, DateTimeFormatter.ofPattern(YMDHMS));
        LocalDateTime localDateTime2 = LocalDateTime.parse(dateTime2, DateTimeFormatter.ofPattern(YMDHMS));
     ...
     * @return yyyy-MM-dd HH:mm:ss
     */
    public static String getLastDateTimeForDate(String dateStr) {
        LocalDate date = LocalDate.parse(dateStr, DateTimeFormatter.ofPattern(YMD));
     ...
     * @return yyyy-MM-dd HH:mm:ss
     */
    public static String getTodayFirstDateTimeStr() {
     ...
     * @return yyyy-MM-dd HH:mm:ss
     */
    public static String getTodayLastDateTimeStr() {
        LocalDate date = LocalDate.now();
        LocalDateTime dateTime = LocalDateTime.of(date, LocalTime.MAX);
        return dateTime.format(DateTimeFormatter.ofPattern(YMDHMS));
    }
    ...
    public static void main(String[] args) {
        // System.out.println(formatDate(getStartDateOfCurrentDay(new Date())));
        System.out.println(getMaxDayOfCurrentMonth());
        System.out.println(formatDateChineseYmdh(new Date()));
        System.out.println(getYearAndMonthYesterday());
        System.out.println(getYearOfNow());
        System.out.println(getDaysByYearMonth(2021, 5));
    }
